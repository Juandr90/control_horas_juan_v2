// Control Horas Juan v2 - app_v2.js (Export CSV versión)
const BASE_HOURS = 8;
...
function startChrono(which, iso){ 
  if(which==='m'){ ch_m_start=iso; localStorage.setItem('ch_m_start', iso); startMInterval(); } 
  else { ch_a_start=iso; localStorage.setItem('ch_a_start', iso); startAInterval(); } 
}
function stopChrono(which){ 
  if(which==='m'){ ch_m_start=null; localStorage.removeItem('ch_m_start'); stopMInterval(); chrono_m.innerText='00:00:00'; } 
  else { ch_a_start=null; localStorage.removeItem('ch_a_start'); stopAInterval(); chrono_a.innerText='00:00:00'; } 
}

function startMInterval(){ 
  if(mInterval) clearInterval(mInterval); 
  mInterval=setInterval(()=>{ 
    if(!ch_m_start) return; 
    const diff=Math.floor((Date.now()-new Date(ch_m_start))/1000); 
...
function startAInterval(){ 
  if(aInterval) clearInterval(aInterval); 
  aInterval=setInterval(()=>{ 
    if(!ch_a_start) return; 
    const diff=Math.floor((Date.now()-new Date(ch_a_start))/1000); 
...
function computeTotals(d){ 
  const r=getRec(d); 
  if(!r) return {totalMins:0,totalHours:0,extra:0}; 
  const mM=r.in_m&&r.out_m?timeDiffInMinutes(r.in_m,r.out_m):0; 
  const aM=r.in_a&&r.out_a?timeDiffInMinutes(r.in_a,r.out_a):0; 
  const totalM=Math.max(0,mM)+Math.max(0,aM); 
  const totalH=+(totalM/60).toFixed(2); 
  const extra=Math.max(0, +(totalH-BASE_HOURS).toFixed(2)); 
  return {totalMins:totalM,totalHours:totalH,extra}; 
}

function renderSummary(d){ 
  const t=computeTotals(d); 
  total_today.innerText = t.totalMins>0? minsToHHMM(t.totalMins)+` (${t.totalHours} h)` : '--'; 
...
function renderTable(){ 
  ...
  records.forEach(r=>{ 
    ...
    <td>${r.extra_hours||0} h</td>
    <td><button class="small-btn" onclick="loadForEdit('${r.date}')">Editar</button> 
    <button class="small-btn danger" onclick="deleteRecord('${r.date}')">Borrar</button></td>`;
...
function loadForEdit(d){ 
  const r=getRec(d); 
  if(!r) return; 
  datePicker.value=r.date; 
  renderSummary(r.date); 
  alert('Fecha cargada.'); 
}
function deleteRecord(d){ 
  if(!confirm('Eliminar '+d+'?')) return; 
  records=records.filter(x=>x.date!==d); 
  save(); 
  renderTable(); 
...
function recordAction(type){ 
  ...
  let r=getRec(d); 
  if(!r){ r={date:d,in_m:null,out_m:null,in_a:null,out_a:null,total_hours:0,extra_hours:0}; records.push(r);} 
  const now=new Date(); 
  const t=now.toTimeString().slice(0,8); 
  if(type==='in_m'){ 
    if(r.in_m){ alert('Ya entrada mañana.'); return;} 
    r.in_m=t; 
    startChrono('m', now.toISOString()); 
  } 
  else if(type==='out_m'){ 
    if(!r.in_m){ alert('No hay entrada mañana.'); return;} 
    if(r.out_m){ alert('Ya salida mañana.'); return;} 
    r.out_m=t; 
    stopChrono('m'); 
  } 
  else if(type==='in_a'){ 
    if(r.in_a){ alert('Ya entrada tarde.'); return;} 
    r.in_a=t; 
    startChrono('a', now.toISOString()); 
  } 
  else if(type==='out_a'){ 
    if(!r.in_a){ alert('No hay entrada tarde.'); return;} 
    if(r.out_a){ alert('Ya salida tarde.'); return;} 
    r.out_a=t; 
    stopChrono('a'); 
    alert('Día guardado automáticamente ✅'); 
  }
  const totals=computeTotals(d); 
  r.total_hours=totals.totalHours; 
  r.extra_hours=totals.extra; 
  save(); 
  renderTable(); 
  renderSummary(d); 
}
...
function exportMonth(){
  ...
  const fileName = `Control_Horas_Juan_${monthNames[m-1]}_${y}.csv`;

  // Cabecera
  let csv = 'Fecha;Entrada mañana;Salida mañana;Entrada tarde;Salida tarde;Total horas (h);Horas extra (h)\n';

  // Filas
  filtered.forEach(r=>{
    csv += `${r.date};${r.in_m||''};${r.out_m||''};${r.in_a||''};${r.out_a||''};${r.total_hours||0};${r.extra_hours||0}\n`;
  });
